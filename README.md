...
占用磁盘空间，降低更新效率。  
### 索引分类有哪些？特点是什么？
聚簇索引，辅助索引，单列索引，组合索引，唯一性索引，主键索引，二级索引，普通索引，全文索引，空间索引，前缀索引  
### 索引创建的原则是什么？
频繁出现在where条件，order by, group by 的字段
select 频繁查询的列
...
单表索引的个数不要太多，尤其是单列索引
频繁更新的资源，不建议作为索引
区分度不高的字段，不建议作为单独索引，可以放在组合索引中
无序的字段，不建议作为索引
主键索引建议使用自增的长整形，避免使用很长的字段
尽量使用组合索引，而不是单列索引
### 如何知道 SQL 是否用到了索引？
在SQL语句前加上explain  
### 请你解释一下索引的原理是什么？【重点】
### 说清楚为什么要用 B+Tree
B+Tree 非叶子节点不存储数据，只存储键值，所以磁盘占用空间小，等值查询和范围查询都能快速查找。
## 题目 03- 什么是 MVCC？
### 要点：
